...
def create_model_emb(args, pyreader_name, ernie_config, is_prediction=False):
    pyreader = fluid.layers.py_reader(
        capacity=50,
        shapes=[[-1, args.max_seq_len, 1], [-1, args.max_seq_len, 1],
                [-1, args.max_seq_len, 1], [-1, args.max_seq_len, 1], [-1, 1],
    ...
    # cls_feats = ernie.get_pooled_output()
    emb_feats = ernie.get_sequence_output()
    # cls_feats = fluid.layers.dropout(
    #     x=cls_feats,
    #     dropout_prob=0.1,
    #     dropout_implementation="upscale_in_train")
    ...
    if is_prediction:
        ...
        feed_targets_name = [
            src_ids.name, pos_ids.name, sent_ids.name, input_mask.name, labels.name
        ]
        return pyreader, emb_feats, feed_targets_name
...
def evaluate_emb(exe, test_program, test_pyreader, 
        graph_vars, eval_phase, use_multi_gpu_test=False, flag=''):
    ...
    # ]
    fetch_list = [
        graph_vars["emb_feats"].name,
        graph_vars["feed_targets_name"][0],
        graph_vars["feed_targets_name"][1],
        graph_vars["feed_targets_name"][2],
        graph_vars["feed_targets_name"][3],
        graph_vars["feed_targets_name"][4],
    ]
    # do_auc = True
    # if do_auc:
    #     fetch_list.append(graph_vars["auc"].name)
    ...
    while True:
        try:
            ...
            emb_feats, src_ids, pos_ids, sent_ids, input_mask, labels = exe.run(
                program=test_program, fetch_list=fetch_list) \
                        if not use_multi_gpu_test else exe.run(fetch_list=fetch_list)

            # val_probs += np_probs[:, 1].tolist()
            # val_labels += np_labels.reshape(-1).tolist()
            val_embeds += emb_feats.tolist()
            val_srcids += src_ids.tolist()
            val_labels += labels.reshape(-1).tolist()
    ...
    #                total_acc / total_num_seqs, mrr, map, p, r, f, total_num_seqs,
    #                time_end - time_begin))
    # print("val_embeds", np.array(val_embeds).shape)
    # print("val_srcids", np.array(val_srcids).shape)

    return {'molid':np.array(val_labels,dtype=int), 'srcid':val_srcids, 'embed':val_embeds}