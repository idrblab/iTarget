...
class run():
    ...
    def inits(self, fold, cv_data, id2idx, fea_drug, fea_prot):
        ...
        data_prot_train = fea_prot[train['gdsc-id'].map(id2idx).values]
        data_drug_valid = fea_drug[valid['drugid'].map(id2idx).values]
        data_prot_valid = fea_prot[valid['gdsc-id'].map(id2idx).values]
        data_drug_test = fea_drug[test['drugid'].map(id2idx).values]
        data_prot_test = fea_prot[test['gdsc-id'].map(id2idx).values]
        # reshape for torch 
